...
maybeDescribe('Pre-Release Stress & Concurrency Gate (Sprint S13)', () => {
  ...
  // Test Data IDs
  const STRESS_INGREDIENT_ID = uuidv4();
  const STRESS_MENU_ITEM_ID = uuidv4();
  const STRESS_CATEGORY_ID = uuidv4();
  ...
  beforeAll(async () => {
    ...
    await prismaSidecarLoose(prisma)
  ...
  function calculateMd5Sig(orderId: string, amount: string) {
  ...
  it('Objective 1 & 2: Flash Sale — 100 Orders Concurrency & Inventory Accuracy', async () => {
    const NUM_ORDERS = 4;
    const orderPromises: Array<Promise<{ success: boolean }>> = [];
        ...
        const promise = (async () => {
            ...
            const placeRes = await request(app.getHttpServer())
                ...
                .send(buildWrapOrderFixture({
                  ...
                  payment: { method: ORDER_VALUES.paymentMethod.online, status: ORDER_VALUES.paymentStatus.pending },
    ...
    logger.log(`Inventory Audit: Expected ${expectedStock}, Actual ${Number(ingredient?.currentStock)}`);
    expect(Number(ingredient?.currentStock)).toBe(expectedStock);
  ...
  it('Objective 3: Offline Cashier Sync — 10 Batch Orders', async () => {
    ...
    const NUM_OFFLINE = 4;
    const syncPromises: Array<Promise<{ status: number }>> = [];
    ...
    for (let i = 0; i < NUM_OFFLINE; i++) {
        ...
        const promise = request(app.getHttpServer())
            ...
            .send(buildWrapOrderFixture({
              orderId,
              status: ORDER_VALUES.status.paid,
              source: ORDER_VALUES.source.cashierOffline,
