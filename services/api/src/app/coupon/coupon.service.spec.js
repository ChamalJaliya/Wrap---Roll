...
function makePrisma(overrides = {}) {
    var _a, _b, _c;
    return {
        coupon: Object.assign(Object.assign({}, defaultCouponMock()), ((_a = overrides.coupon) !== null && _a !== void 0 ? _a : {})),
        order: Object.assign(Object.assign({}, defaultOrderMock()), ((_b = overrides.order) !== null && _b !== void 0 ? _b : {})),
        opsActivityEvent: Object.assign(Object.assign({}, defaultOpsMock()), ((_c = overrides.opsActivityEvent) !== null && _c !== void 0 ? _c : {})),
    };
}
...
function activeCoupon(overrides = {}) {
    return Object.assign({ id: 'coupon-1', code: 'WELCOME10', discountPercent: 0.1, minSubtotal: null, firstOrderOnly: false, isActive: true, expiryDate: null }, overrides);
}
// ---------------------------------------------------------------------------
// validateCoupon
// ---------------------------------------------------------------------------
describe('CouponService.validateCoupon', () => {
    it('returns invalid when code is blank', async () => {
        const svc = new coupon_service_1.CouponService(makePrisma());
    ...
    it('returns invalid when coupon does not exist', async () => {
        ...
        const svc = new coupon_service_1.CouponService(prisma);
    ...
    it('normalises code to UPPERCASE before lookup', async () => {
        ...
        expect(couponMock.findUnique).toHaveBeenCalledWith(expect.objectContaining({ where: { code: 'WELCOME10' } }));
    });
...
describe('CouponService.createAdmin', () => {
    it('throws when code is empty', async () => {
        const svc = new coupon_service_1.CouponService(makePrisma());
        await expect(svc.createAdmin({ code: '', discountPercent: 0.1 }, mockActor)).rejects.toThrow(common_1.BadRequestException);
    });
    it('throws when discountPercent is out of range', async () => {
        const svc = new coupon_service_1.CouponService(makePrisma());
        await expect(svc.createAdmin({ code: 'OVER100', discountPercent: 1.5 }, mockActor)).rejects.toThrow(common_1.BadRequestException);
    });
    it('creates coupon and writes audit trail', async () => {
        ...
        const prisma = makePrisma({
            coupon: Object.assign(Object.assign({}, defaultCouponMock()), { create: createFn }),
            opsActivityEvent: { create: opsFn },
        });
        const svc = new coupon_service_1.CouponService(prisma);
        const result = await svc.createAdmin({ code: 'LAUNCH', discountPercent: 0.15 }, mockActor);
        expect(result).toBe(created);
        expect(createFn).toHaveBeenCalledTimes(1);
        expect(opsFn).toHaveBeenCalledWith(expect.objectContaining({
...
describe('CouponService.updateAdmin', () => {
    it('throws NotFoundException when coupon does not exist', async () => {
        const prisma = makePrisma({ coupon: { findUnique: jest.fn().mockResolvedValue(null) } });
        const svc = new coupon_service_1.CouponService(prisma);
        await expect(svc.updateAdmin('nonexistent-id', { isActive: false }, mockActor)).rejects.toThrow(common_1.NotFoundException);
    });
    it('updates and writes audit trail', async () => {
        const existing = { id: 'c-2', code: 'SALE', isActive: true };
        const updated = Object.assign(Object.assign({}, existing), { isActive: false });
...
describe('CouponService.deleteAdmin', () => {
    it('throws NotFoundException when coupon does not exist', async () => {
        const prisma = makePrisma({ coupon: { findUnique: jest.fn().mockResolvedValue(null) } });
        const svc = new coupon_service_1.CouponService(prisma);
        await expect(svc.deleteAdmin('ghost-id', mockActor)).rejects.toThrow(common_1.NotFoundException);
    });
