...
function makeIngredient(overrides = {}) {
    return Object.assign({ id: ING_ID, name: 'Chicken', unit: 'g', currentStock: decimalOf(1000), costPerUnit: decimalOf(1.5), lowStockThreshold: decimalOf(100) }, overrides);
}
// ---------------------------------------------------------------------------
...
describe('InventoryService.restockIngredient', () => {
    it('calls $transaction and returns a movement record', async () => {
        ...
        const transactionFn = jest.fn().mockImplementation(async (cb) => cb(txMock));
        ...
        const svc = new inventory_service_1.InventoryService(prisma);
        const result = await svc.restockIngredient({ ingredientId: ING_ID, quantity: 100, unitCost: 2 }, mockActor);
    ...
    it('rejects with ZodError for a non-UUID ingredientId (schema-level guard)', async () => {
        const svc = new inventory_service_1.InventoryService(makePrisma());
        await expect(svc.restockIngredient({ ingredientId: 'not-a-uuid', quantity: 100, unitCost: 2 }, mockActor)).rejects.toBeDefined();
    });
});
// ---------------------------------------------------------------------------
...
describe('InventoryService — low stock alert', () => {
    it('allows movement and logs low-stock warning when threshold crossed', async () => {
        ...
        const svc = new inventory_service_1.InventoryService(prisma);
        await svc.recordWaste({ ingredientId: ING_ID, quantity: 10, note: 'Spilled' }, mockActor);
...
describe('InventoryService — insufficient stock guard', () => {
    it('throws when waste exceeds currentStock (Zod passes, appendMovement rejects)', async () => {
        ...
        await expect(svc.recordWaste({ ingredientId: ING_ID, quantity: 10 }, mockActor)).rejects.toBeDefined(); // ZodError or BadRequestException
    });
});
// ---------------------------------------------------------------------------
...
describe('InventoryService.createIngredient — validation', () => {
    it('throws when lowStockThreshold exceeds initialStock', async () => {
        ...
        const svc = new inventory_service_1.InventoryService(prisma);
        await expect(svc.createIngredient({ name: 'Salt', unit: 'g', currentStock: 10, costPerUnit: 0.5, lowStockThreshold: 50 }, mockActor)).rejects.toThrow(common_1.BadRequestException);
    });
});
// ---------------------------------------------------------------------------
